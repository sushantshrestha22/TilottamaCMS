...
const UpdateAdvisory = () => {
  ...
  const submitFormData = async (formData, id) => {
    const response = await editData(`/api/advisory-committee/${id}`, formData);
  ...
  const {
    ...
    queryKey: ["advisory", id], // Ensure queryKey is an array
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["advisory", id]);
      toast.success("Advisory committee Updated successfully");
