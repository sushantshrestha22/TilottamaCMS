...
const CreateResearchManagement = () => {
  ...
  if (!isLoggedIn) {
    ...
    const submitFormData = async (formData) => {
      const response = await postData("/api/research-management", formData);
      return response.data; // Or whatever your API returns
    };

    const mutation = useMutation({
      mutationKey: ["management"], // Ensure queryKey is an array
      mutationFn: submitFormData,
      onSuccess: () => {
        toast.success("Research management created successfully");
...
export default CreateResearchManagement;
